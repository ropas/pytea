...
pyright의 pyrightconfig.json과 비슷하게 json 파일로 PyTEA의 주요 세팅을 설정할 수 있다. 옵션은 `pytOptions.ts`를 참고할 것

- `pytLibPath`: LibCall들의 Python 구현체들이 들어있는 `pylib` 폴더를 가리키는 경로
- `entryPath`: 시작 Python 파일 경로. 세팅 안할 시 기본적으로 command line으로 받음
- `pythonCmdArgs`: argparse를 위해 세팅하는 python commandline argument. (`python test --batch=16`에서 `test --batch=16` 부분)
  - raw string으로 주지 말고 JSON Object 형태로 주어야 한다. (`{ "batch": "16" }` 같이 )
- `logLevel`: 프론트엔드 로그의 출력 세팅을 조절

  - "none": JSON만 출력함
  - "result_only": 에러 출력 또는 성공한 path 갯수를 출력
  - "reduced" (default): reduced heap을 출력 (global variable의 값만 출력)
  - "full": env, heap을 모두 출력

- `immediateConstraintCheck`: 이게 있으면 symbolic expression과 constraint를 frontend에서 처리하지 않으며 모든 potential path를 전부 생성.

## Requirements

다음 라이브러리와 버전을 기반으로 구현
